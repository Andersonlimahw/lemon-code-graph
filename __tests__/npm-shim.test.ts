...
// A temp dir standing in for the installed @andersonlimahw/lemon-codegraph main package.
...
describe.skipIf(isWindows)('npm-shim launcher', () => {
  it('runs the installed optional-dependency bundle without any download', async () => {
    const pkg = makePkg();
    const platformPkg = path.join(pkg, 'node_modules', '@andersonlimahw', `lemon-codegraph-${target}`);
  ...
  it('prints actionable guidance and exits 1 when disabled with no bundle', async () => {
    ...
    expect(r.stderr).toContain(`@andersonlimahw/lemon-codegraph-${target}`);
